...
export default function Invoice() {
  ...
  const [prevCharge,  setPrevCharge]    = useState('');  // 前回御請求額
  const [prevPayment, setPrevPayment]   = useState('');  // 前回御入金額
  const [adjustment,  setAdjustment]   = useState('');  // 調整額
    ...
    <div style={{ padding: '1rem', maxWidth: 960, margin: '0 auto' }}>
      ...
      {currentClinic && (
        <div style={{ background: '#f9f9f9', borderRadius: 8, padding: '0.75rem 1rem', marginBottom: '1rem', fontSize: 13 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
            ...
            {!editClinic && (
              <button onClick={() => setEditClinic({
                postalCode: currentClinic.postalCode || '',
                address:    currentClinic.address    || '',
                bankInfo:   currentClinic.bankInfo   || '',
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'flex-end' }}>
              ...
              <label style={{ display: 'flex', flexDirection: 'column', fontSize: 12, gap: 2 }}>
                ...
                <input value={editClinic.bankInfo} onChange={e => setEditClinic(p => ({ ...p, bankInfo: e.target.value }))}
                  style={{ ...inputSt, width: 280 }} placeholder="みずほ銀行 京都支店 普通 1234567" />
              </label>
        ...
        {historyLoading ? (
          ...
          <>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'flex-end' }}>
              ...
              <div style={{ alignSelf: 'flex-end', color: '#1a5c3a', fontWeight: 'bold', fontSize: 14 }}>
      ...
      <div style={{ marginBottom: '1rem' }}>
        <button onClick={handleCreate} disabled={loading || historyLoading}
          style={{ padding: '9px 28px', background: '#1a5c3a', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 'bold', fontSize: 15 }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.25rem' }}>
            {[
              ...
              <div key={label} style={{ background: val < 0 ? '#fff0f0' : '#f0f9f4', borderRadius: 8, padding: '0.5rem 1rem', minWidth: 120, textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 16, fontWeight: 'bold', color: val < 0 ? '#c0392b' : '#1a5c3a' }}>¥{val.toLocaleString()}</div>
              </div>
            ))}
          </div>
          <InvoicePrint
